...
void Binary_Traversal_Levelorder(_binaryTree* tree){
	if(tree->root == NULL){
		return;
	}
	_node* current_node = NULL;
	_queue* queue = Queue_Init();
	Queue_Insert(queue, tree->root);

	while(queue->front != NULL){
		current_node = (_node*)Queue_Remove(queue);
		printf("current node: %d\n", current_node->data);
		printf("height: %d\n", current_node->height);
		printf("balance: %d\n\n", current_node->balance);
		if(current_node->left_child != NULL){
			Queue_Insert(queue, current_node->left_child);
		}
		if(current_node->right_child != NULL){
			Queue_Insert(queue, current_node->right_child);
		}
	}
}
